...
{
    [Fact]
    public void MobileExportUsesUserInitiatedFallbackAndRejectsUnreliablePickers()
    {
        ...
        script.Should().Contain("return /SamsungBrowser\\//i.test(navigator.userAgent);");
        script.Should().Contain("window.pkmdsSupportsSaveFilePicker = function ()");
        script.Should().Contain("!pkmdsIsSamsungInternet();");
