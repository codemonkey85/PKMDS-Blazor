...
public class PokemonData
{
    ...
    private void SetValue(byte value, int destinationIndex) => Array.Copy(BitConverter.GetBytes(value), 0, RawData, destinationIndex, sizeof(byte));
    private void SetValue(ushort value, int destinationIndex) => Array.Copy(BitConverter.GetBytes(value), 0, RawData, destinationIndex, sizeof(ushort));
    private void SetValue(ulong value, int destinationIndex) => Array.Copy(BitConverter.GetBytes(value), 0, RawData, destinationIndex, sizeof(ulong));
    ...
    public bool IsFatefulEncounter
    {
        ...
        set
        {
            BitArray ba = new(new byte[] { RandomStuff });
            ba.Set(0, value);
    ...
    public byte FormeIndex
    {
        ...
        {
            BitArray ba = new(new byte[] { RandomStuff });
            BitArray tempBa = new(new byte[] { value });
    ...
    public override string ToString() => $"{NationalId} {Pid} {Tid} {Sid} {IsFemale} {IsGenderless} {FormeIndex} {IsShiny}";
}
